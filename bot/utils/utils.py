...
from bot.config import ADMIN_ID, CHANEL_ID, REDIS_PORT
import redis
import logging
...
redis_client = redis.Redis(port=REDIS_PORT)

def set_work_mode(mode):
    try:
        with redis.Redis(port=REDIS_PORT) as redis_client:
...
def get_sample_from_article(article: Article):
    try:
        ...
        return text
            ...
            logger.exception('Get sample from article')
...
async def check_article_for_errors(article: Article):
    try:
        ...
        return
            ...
            logger.exception('Check article for errors')
